...
//	storage['consigneName']=$('input[name=name]').val();
//	storage['consignePhone']=$('input[name=name]').val();
//	storage['consigneAddress']=$('input[name=address]').val();
//	storage['consigneAdrDetail']=$('#textarea').val();
	var adrId=$.getUrlParam('adrId');
	var adr1=storage['consigneAddress'].split('}')
	$('input[name=userId]').val(storage['getUserId']);
	$('input[name=sign]').val(storage['getSign']);
	$('input[name=name]').val(storage['consigneName'])
	$('input[name=phone]').val(storage['consignePhone'])
	$('input[name=id]').val(storage['adrId'])
	$('#aa').text(adr1[0])
	console.log($('#aa').text())
	$('textarea').text(adr1[1])
	var proData='';
	var str='';
		$.ajax({
			type:"get",
		...
		$('#add_btn').click(function(){
			if($('input[name=name]').val()==''){
				...
				$.ajax({
					type:"post",
					url:apiUrl+"/member/updateaddress",
