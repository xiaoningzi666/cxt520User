		...
		success:function(data){
			...
			$.each(data.pageInfo.list, function(index,item) {
				html+='<div class="mui-row activtyList_wrap">'+
					        '<div class="mui-col-xs-8 actPadding">'+
					        	'<a href="../content/contentdetail.jsp?id='+item.id+'">'+
					        		'<h4>'+item.title+'</h4>'+
					        		'<span style="color:#888">'+timeFormat(item.releaseTime)+'</span>'+
					        	'</a>'+
					        '</div>'+
					        '<div class="mui-col-xs-4 actPadding">'+
					       		'<a href="../content/contentdetail.jsp?id='+item.id+'">'+
					       			'<img src="'+item.imageList[0].url+'" width="100%"/>'+
					       		'</a>'+
					       ' </div>'+
					    '</div>'
			});
