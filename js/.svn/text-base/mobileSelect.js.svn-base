...
window.MobileSelect = (function() {
	...
	MobileSelect.prototype = {
			...
			for(var i=0; i<wheelsData.length; i++){ //列
				...
				if(_this.jsonType){
					for(var j=0; j<wheelsData[i].data.length; j++){ //行
						tempHTML += '<li data-id="'+wheelsData[i].data[j].id+'">'+wheelsData[i].data[j].name+'</li>';
					}
				}else{
					for(var j=0; j<wheelsData[i].data.length; j++){ //行
		...
		checkCascade: function(){
			...
			if(_this.jsonType){ 
				...
				for(var i=0; i<node.length; i++){
					if('regionList' in node[i] && node[i].regionList.length > 0){
						_this.cascade = true;
						break;
					}
				}
		...
		checkArrDeep: function (parent) { //检测子节点深度  修改 displayJson
			var _this = this;
			if ('regionList' in parent && parent.regionList.length > 0) {
				_this.displayJson.push(_this.generateArrData(parent.regionList)) ; //生成子节点数组
				_this.checkArrDeep(parent.regionList[0]);//检测下一个子节点
			}
		},
		...
		checkRange: function(index, posIndexArr){
			...
			for (var i = 0; i <= index; i++){
				if (i == 0)
					...
					resultNode = resultNode.regionList[posIndexArr[i]];
				}
			}
